...
- **Native share sheet on Android and iOS**
  ...
  clipboard, letting players send their round straight to Messages, WhatsApp, or
  other apps. Desktop browsers (macOS Safari, Windows Chrome) continue copying
  directly to the clipboard with the existing snackbar to avoid popping intrusive
  desktop OS dialogs. User dismissals (`AbortError`) cancel cleanly without
  triggering false error or fallback states, while actual failures degrade
  gracefully to clipboard copy and the manual `<TextField>` fallback.
